...
public abstract class BasicCommand<C extends SharedSuggestionProvider> {
    static void registerServerSideArguments() {
        ...
        Services.PLATFORM.registerArgumentType(Identifier.parse("fix-mc-stats:patchaction"), PatchActionArgumentType.class, SingletonArgumentInfo.contextAware(access -> PatchActionArgumentType.pathAction()));
        Services.PLATFORM.registerArgumentType(Identifier.parse("fix-mc-stats:patch"), PatchArgumentType.class, SingletonArgumentInfo.contextAware(access -> PatchArgumentType.patchArgument(ModEnvironment.SERVER)));
